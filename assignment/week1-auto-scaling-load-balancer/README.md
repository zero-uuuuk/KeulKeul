...
| 단계 | 시간 | 부하 정도 | 내용 |
| --- | --- | --- | --- |
| 워밍업 | 3분 | 동시 요청 5개, 요청당 CPU 작업 120ms | 낮은 부하로 시작해 ALB와 서버 응답이 정상인지 확인합니다. |
| 폭증 | 6분 | 동시 요청 80개, 요청당 CPU 작업 300ms | 높은 부하를 발생시켜 ASG의 scale-out을 유도합니다. |
| 정상 복귀 | 3분 | 동시 요청 5개, 요청당 CPU 작업 120ms | 요청 강도를 워밍업 수준으로 낮춰 서버가 안정 상태로 돌아가는지 확인합니다. |
| scale-in 관찰 | 15분 | 요청 없음 | 요청을 보내지 않고 서버 수가 다시 줄어드는지 관찰합니다. |
